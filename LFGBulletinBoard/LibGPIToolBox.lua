...
function Tool.GuildNameToIndex(name, searchOffline)
	...
	for i = 1,GetNumGuildMembers(searchOffline) do
		...
		if rosterName and string.lower(string.match(rosterName,"^([^%-]+)") or rosterName) == name then
			return i
		end
	end
end
...
function Tool.UnitDistanceSquared(uId)
	...
	if UnitIsUnit(uId, "player") then
		...
		if type(UnitDistanceSquared)=="function" then distanceSquared, checkedDistance = UnitDistanceSquared(uId) end
		if checkedDistance then
			range=distanceSquared
		elseif UnitIsVisible and not UnitIsVisible(uId) then
			range = 1000000
...
local SizingEnter=function(self)
	if not (GetCursorInfo()) then
		...
		if ResizeCursor.Texture.SetRotation then ResizeCursor.Texture:SetRotation(math.rad(self.GPI_Rotation),0.5,0.5) end
	end
end
