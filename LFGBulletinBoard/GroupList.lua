...
function GBB.AddGroupList(entry)
	...
	if not guildcache[entry.name] then
		guildcache[entry.name]=GBB.Compat.IsGuildMemberName(entry.name)
	end
	if not friendcache[entry.name] then
		friendcache[entry.name]=GBB.Compat.IsFriendName(entry.name)
	end
...
function GBB.InitGroupList()
	...
	if GroupBulletinBoardFrame_GroupFrame.SetHyperlinksEnabled then GroupBulletinBoardFrame_GroupFrame:SetHyperlinksEnabled(true) end
	...
	if GroupBulletinBoardFrame_GroupFrame.SetTextCopyable then GroupBulletinBoardFrame_GroupFrame:SetTextCopyable(true) end
	GroupBulletinBoardFrame_GroupFrame:Clear()
...
function GBB.ScrollGroupList(self,delta)
	...
	self:SetScrollOffset(getOffset(self) + delta*5);
