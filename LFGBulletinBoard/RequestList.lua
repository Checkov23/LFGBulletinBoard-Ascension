...
function GBB.UpdateList()
	...
	local scrollBar=GroupBulletinBoardFrame_ScrollFrame.ScrollBar or _G["GroupBulletinBoardFrame_ScrollFrameScrollBar"]
	if scrollBar then scrollBar.scrollStep=itemHight*2 end
...
function GBB.ParseMessage(msg,name,guid,channel)
	...
	if engClass==nil or engClass=="" then engClass=GBB.Compat.GetClassByName(name) end
		...
		for dungeon,id in pairs(dungeonList) do
			...
			if id== true and dungeon~=nil then
				...
				if index==0 then
					...
					GBB.RequestList[index].IsGuildMember=GBB.Compat.IsGuildMemberName(name)
					GBB.RequestList[index].IsFriend=GBB.Compat.IsFriendName(name)
	...
	if dungeonTXT~="" and GBB.AllowInInstance() then
		if GBB.DB.NotifyChat then
			local FriendIcon=(GBB.Compat.IsFriendName(name) and string.format(GBB.TxtEscapePicture,GBB.FriendIcon) or "") ..
						 (GBB.Compat.IsGuildMemberName(name) and string.format(GBB.TxtEscapePicture,GBB.GuildIcon) or "") ..
		...
		if GBB.DB.NotifySound then
			GBB.Compat.PlayNotifySound(GBB.NotifySound)
		end
	end
...
function GBB.RequestShowTooltip(self)
	for id in string.gmatch(self:GetName(), "GBB.Item_(.+)") do
		...
		local tip=""
		if n.IsTruncated and n:IsTruncated() then
